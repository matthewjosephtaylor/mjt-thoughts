...
can work with the answer to the question. There is the small complexity that the
answer is only good _inside_ of the context, but perhaps one shouldn't look gift
horses too closely in the mouth. :)
...
There are a number of ways of doing this but the important thing to note is that
there is no guarantee that the resulting 'answer' will be to either Alice or
Bob's liking.

For example `1 + 2 = 3`.

`3` is _an answer_ to how to combine `1` + `2` and sort of 'contains' `1` and
`2` inside of it.
...
to get worse the more individuals are involved.
...
There are broadly speaking two solutions to this problem as I see it.
...
what one chooses for 'the answer', as long as all the individuals accept it as
their own. From first, addition, multiplication, etc.. are _all the same_ and
provide _the same result_ (do the 'math', a swirl with all vanilla and vanilla,
two cones for the price of two, etc... )).

This I think answers the question as to why my more 'socialistic' friends are
the ones so enamored with 'pushing' the acceptance of 'their ideas' on others
(think my way, the right way, ...or else), and my more 'individualistic' friends
usually _don't_ engage in as much political speech (except perhaps to speak out
against being pushed around by 'socialists')).

BTW if one thinks that the other side _should_ think this or that way, then I'd
categorize this as a form of _socialism_ since it is a tendency to desire a
...
is more a choice of if addition, multiplication, etc... is _the_ preferred
solution.
...
the byte...which contains bits. I can't see anything beyond the bit, but I
suspect that is merely a failure of my imagination.
...
I have my opinion on each of these things. They might be weakly held, but they
aren't arbitrary. Likely they are different than yours in some small or large
degree.
...
One answer is that _natural selection_ decides, by allowing me the ability to
perceive only those answers that were capable of continuing in time.

I say it this odd way because there is a funny thing in physics. Nothing is
'real' in the way one may naively think. It is sort of like each individual
particle is a Monad itself. It needs some _context_ to get any sort of _real_
value.
...
So there is a sort of 'real' and 'imaginary' part of the universe, with my
consciousness being limited to perceiving the 'real' one (Note that I'm a good
physicalist and buy into relativity and quantum mechanics, nothing 'spooky'
going on, the physical universe just has some odd aspects to it).

So looking at it this way it is conceivable to think of _all_ answers being
tried, and in some universes Alice is happy, some only Bob is happy, some
neither, some both. The one that Carol (Alice and Bob's child) observes many
years later is impacted more by which ice cream flavor gave the sum-total best
odds of her being conceived and living to the age where Alice and Bob could tell
the tale of what flavor(s) they received).

If I had to put money down, this would be the answer I'd bet on. The answer I'd
_prefer_ is the one where Alice and Bob are happy AND not too much ice-cream
...
effort into what you think the 'right' things are as possible. Hopefully that
means more thoughtful social media posts :) , but I urge 'thoughtful action
towards the good' in all activities in general.
